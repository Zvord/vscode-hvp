...
- Syntax highlighting for `.hvp` files (keywords, types, built-in metrics, strings,
  ...
  `hvp-language-server`'s grammar generator (see "Syntax grammar" below). Inside a
  `source = "..."` string the Table 4 keyword prefix (`group instance bin:`,
  `property categoryMask 'h00f:`, …), the `` `r` ``/`` `n` ``/`` `-` `` tags, the
  `?`/`*`/`**` wildcards and `${name}` are scoped separately from the string; so are
  `enum {...}` members, `aggregate {X(weight=...)}` members and `#(...)` subplan
  parameters.
